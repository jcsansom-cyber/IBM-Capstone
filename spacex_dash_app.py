...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        ...
        names='class count', 
        title='Success of Selected Launch Site')
        return fig
...
def get_scatter(entered_site, pr):
    filtered_df = spacex_df[(spacex_df['Payload Mass (kg)'] >= pr[0]) & (spacex_df['Payload Mass (kg)'] <= pr[1])]
    if entered_site == 'ALL':
        fig = px.scatter(filtered_df, x='Payload Mass (kg)', 
        y='class', 
        color="Booster Version Category",
        title='Payload Mass vs All Sites')
        return fig
    else:
